...
def accept_request_app(client, sender, lender_mnemonic_key,  lender_app_id, request_app_id, l_amount, is_counter_offer):

    # define the sender for the transactions
    print(f'Lender {lender_app_id} interested in Request : {request_app_id}  ...')
    private_key = mnemonic.to_private_key(lender_mnemonic_key)
    ...
    print('borrower address  ', borrower_address)
    print('senders address  ', sender)

    # encoded
    # senders = algosdk.encoding.decode_address(sender)
    # borrower_address = algosdk.encoding.decode_address(borrower_address)
    # print(senders)

    args_list = ["Accept by lender", int(is_counter_offer), int(l_amount), borrower_address, sender]
...
def borrower_counter_app(client, sender, mnemonic_key, lender_accept_txn_id, request_app_id, l_amount, b_amount, is_counter_offer):

    # define the sender for the transactions
    print(f'Borrower responding to lenders counter offer : {lender_accept_txn_id}  ...')
    private_key = mnemonic.to_private_key(mnemonic_key)

    lender_address = get_address_from_txid(lender_accept_txn_id)
    ...
    args_list = ["borrower_counter", is_counter_offer, int(b_amount), sender, lender_address, int(l_amount)]
    txn = ApplicationNoOpTxn(sender=sender, sp=params, index=request_app_id, app_args=args_list)
...
def return_by_borrower(client, sender, mnemonic_key, lender_address, request_app_id, accept_request_txn_id, amount, roi, maturity_date):
    ...
    if int(Today_seconds()) >= int(convert_to_unix_timestamp(maturity_date)) and lender_address == derived_lender_address:
        ...
        try:
            ...
            # args_list = ["Return borrowed amount", int(amount), lender_address, derived_lender_address, sender]
            # txn2 = ApplicationNoOpTxn(sender=sender, sp=params, index=request_app_id, app_args=args_list)
